...
test_that("Can add taxon names with max_rel_abundance method", {
    expect_no_error(urt %>% add_taxon_name(method="max_rel_abundance"))
    expect_no_error(urt %>% add_taxon_name(method="max_rel_abundance", include_species=TRUE))
    # TODO: find some more clever validation here 
})
