...
This vignette explains how to perform some common amplicon data analysis tasks using tidyamplicons. I will illustrate those using a dataset with human microbiome samples from the upper respiratory tract (URT). It contains nose as well as nasopharynx samples. Most samples were taking using a swab method, but a minority was taking with the aspirate method. 

# Philosophy of the package

A tidyamplicons object is simply a list of three tables:

* **abundances**: These are the counts of reads for each taxon in each sample. Each row represents such a read count. 
...
* The otu_table object of phyleq is not tidy, while the abundances table of tidyamplicons is. This tidiness sacrifices some memory and speed, while in return gaining a data format that is easier to process and visualize with a few lines of code. 
...
Let's load our example data set and inspect it: 
...
We can select a subset of our samples by filtering the samples table and then applying the function `process_sample_selection()` to the whole tidyamplicons object. Let's select all nose samples taken with a swab method: 

```{r}
urt_subset <- urt
urt_subset$samples <- urt_subset$samples %>%
  filter(location == "N", method == "S")
urt_subset <- process_sample_selection(urt_subset)
```

The purpose of the sample selection processing is to delete the samples we removed from the samples table also from the abundances table and even the taxa table (taxa that occur in zero samples are removed). 

Now we can very easily visualize these samples by just calling the `get_bar_plot()` function: 

```{r}
get_bar_plot(urt_subset)
```

This function returns a nice barplot visualization of the most abundant taxa in our samples. In addition, it does some things behind the screens: 
...
```{r}
urt_subset <- add_rel_abundance(urt_subset) # adds to abundances table
urt_subset <- add_max_rel_abundance(urt_subset) # adds to taxa table
urt_subset <- add_taxon_name(urt_subset) # adds to taxa table
urt_subset <- add_taxon_name_color(urt_subset) # adds to taxa table
```
...
urt2 <- process_taxon_selection(urt2)
```
...
```{r}
urt2 <- add_taxon_name_color(urt2, n = 12)
```
